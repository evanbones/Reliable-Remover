...
Also supports targeting specific `items` to allow or preserve enchantments on specific tools.

* **Use Case**: Remove "Protection" or specific curses from loot, enchanting tables, villager trades, and books entirely, or remove an enchantment from all items except specific tools.
...
#### Whitelisting Specific Items (`not`):
...
You can also target items generated with specific enchantments:
